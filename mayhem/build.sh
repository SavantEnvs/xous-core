...
# Upstream ships the cargo-fuzz crate at apps/vault/libraries/cbor/fuzz.
...
# mayhem/fuzz-cbor is an unmodified copy of apps/vault/libraries/cbor/fuzz at this
# commit (same libfuzzer-sys 0.3 dependency); it lives under mayhem/ only so the
# era-matched nightly (RUST_CHANNEL in mayhem/Dockerfile) builds it in isolation
# from the workspace's own toolchain pin.
